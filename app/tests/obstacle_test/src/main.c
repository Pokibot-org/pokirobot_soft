...
ZTEST(obstacles_test, test_collision_circles) {
    ...
    zassert_equal(
        1, obstacle_are_they_colliding(&a, &b), "A and B must collide");
    b.data.circle.coordinates.x = 100;
    zassert_equal(
        0, obstacle_are_they_colliding(&a, &b), "A and B must not collide");
}

ZTEST(obstacles_test, test_collision_rectangles) {
    obstacle_t a = {.type = obstacle_type_rectangle,
        .data.rectangle = {
            .coordinates = {.x = 50, .y = 50}, .height = 20, .width = 20}};

    obstacle_t b = {.type = obstacle_type_rectangle,
        .data.rectangle = {
            .coordinates = {.x = 60, .y = 60}, .height = 20, .width = 20}};
...
ZTEST(obstacles_test, test_collision_rectangles_and_circles) {
    ...
    zassert_equal(
        1, obstacle_are_they_colliding(&a, &b), "A and B must collide");
    zassert_equal(
        1, obstacle_are_they_colliding(&b, &a), "A and B must collide");
    b.data.circle.coordinates.x = 100;
    zassert_equal(
        0, obstacle_are_they_colliding(&a, &b), "A and B must not collide");
    zassert_equal(
        0, obstacle_are_they_colliding(&b, &a), "A and B must not collide");
}
...
ZTEST(obstacles_test, test_object_holder) {
    ...
    zassert_equal(0, obstacle_holder_push(&holder, &ob), "Push is not valid");
    zassert_mem_equal(&ob, &holder.obstacles[1], sizeof(obstacle_t),
        "Store object is not valid");
}
...
ZTEST(obstacles_test, test_object_collision_with_seg_and_circle) {
    ...
    int rcode = obstacle_get_point_of_collision_with_segment(
        &start, &end, &ob, robot_radius, &intersection_pt);
    zassert_equal(1, rcode, "Intersection not found");

    ob.data.circle.coordinates.x =
        end.x + ob.data.circle.radius + robot_radius + 1;
    // FIXME: current method is aproximate even working with
    // ob.data.circle.coordinates.x -= 1
    rcode = obstacle_get_point_of_collision_with_segment(
        &start, &end, &ob, robot_radius, &intersection_pt);
...
ZTEST(obstacles_test, test_object_collision_with_seg_and_rectangle) {
    ...
    ob.data.rectangle.coordinates.x =
        end.x + ob.data.rectangle.width / 2 + robot_radius + 1;
    // FIXME: current method is aproximate even working with
    // ob.data.circle.coordinates.x -= 1
    rcode = obstacle_get_point_of_collision_with_segment(
        &start, &end, &ob, robot_radius, &intersection_pt);
...
ZTEST(obstacles_test, test_get_point_collision_two_segments) {
    ...
    zassert_equal(
        expected_out.x, out.x, "Wrongly calculated collision point x");
    zassert_equal(
        expected_out.y, out.y, "Wrongly calculated collision point y");
