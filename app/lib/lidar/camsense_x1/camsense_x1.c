...
void process_recived_frame(uint8_t *payload)
{
    ...
    obj.current_speed =
        ((uint16_t)(payload[CAMSENSE_X1_SPEED_H_INDEX] << 8) | payload[CAMSENSE_X1_SPEED_L_INDEX]) /
        3840.0; // 3840 = (64 * 60)
    message.start_angle = -640 + (((uint16_t)payload[CAMSENSE_X1_START_ANGLE_H_INDEX]) << 8 |
                                  payload[CAMSENSE_X1_START_ANGLE_L_INDEX]) /
                                     64.0; // TODO: Use shift not /
    message.end_angle = -640 + (((uint16_t)payload[CAMSENSE_X1_END_ANGLE_H_INDEX]) << 8 |
                                payload[CAMSENSE_X1_END_ANGLE_L_INDEX]) /
                                   64.0;

    for (int point_index = 0; point_index < NUMBER_OF_LIDAR_POINTS; point_index++) {
        uint8_t distance_l =
            payload[CAMSENSE_X1_FIRST_POINT_INDEX + CAMSENSE_X1_POINT_DISTANCE_L_RELATIVE_INDEX +
                    (point_index * 3)];
        uint8_t distance_h =
            payload[CAMSENSE_X1_FIRST_POINT_INDEX + CAMSENSE_X1_POINT_DISTANCE_H_RELATIVE_INDEX +
                    (point_index * 3)];
...
{
    int err =
        uart_rx_enable(obj.uart_dev, obj.frame_buffer, CAMSENSE_X1_HEADER_SIZE, SYS_FOREVER_MS);
    if (err) {
        LOG_ERR("Cant start full frame read");
    }
}

void camsense_x1_full_frame_callback(const struct device *dev, struct uart_event *evt,
                                     void *user_data)
{
