...
int32_t pokibrain_reward_calulation_grab_smth(struct pokibrain_callback_params *params)
{
	LOG_INF("%s", __func__);
	return 0;
}
...
int32_t pokibrain_reward_calulation_put_smth(struct pokibrain_callback_params *params)
{
	LOG_INF("%s", __func__);
	struct pokibrain_user_context *ctx = params->world_context;
