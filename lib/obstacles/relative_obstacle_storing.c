...
uint8_t relative_obstacle_storing_lidar_points_relative_to_robot(
    obstacle_holder_t* holder, lidar_message_t* message, float robot_angle_rad,
    point2_t robot_pos, float center_offset_degre) {
    float step = 0.0f;
         ...
         i++) // for each of the 8 samples
    {
        ...
        float point_angle_absolute =
            ((point_angle * (M_PI / 180.0f)) + robot_angle_rad);
        ...
        if (message->points[i].quality != 0) // Filter some noisy data
        {
            new_obstacle.data.circle.coordinates.x =
                robot_pos.x +
                sinf(point_angle_absolute) * message->points[i].distance;
            new_obstacle.data.circle.coordinates.y =
                robot_pos.y +
                cosf(point_angle_absolute) * message->points[i].distance;
