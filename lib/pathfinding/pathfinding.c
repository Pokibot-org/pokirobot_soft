...
static uint8_t get_closest_point_of_collision(const pathfinding_object_t* obj,
    const obstacle_holder_t* ob_hold, const point2_t* rand_coordinates,
...
int pathfinding_object_configure(
    pathfinding_object_t* obj, pathfinding_configuration_t* config) {
    // TODO: Check if delta is under distance to goal
    obj->config = *config;
    if ((obj->config.field_boundaries.max_x ==
...
path_node_t* get_closest_node(pathfinding_object_t* obj, point2_t* crd) {
    ...
    for (size_t i = 0; i < obj->next_free_node_nb; i++) {
        if (obj->nodes[i].is_used) {
            uint32_t distance =
                vec2_abs(point2_diff(obj->nodes[i].coordinate, *crd));
...
uint8_t get_array_of_closest_node(
    pathfinding_object_t* obj, point2_t* crd, path_node_t** out_nodes) {
    uint8_t found_nodes = 0;
    for (size_t i = 0; i < obj->next_free_node_nb; i++) {
        path_node_t* current_node = &obj->nodes[i];
        if (current_node->is_used) {
            float distance =
                vec2_abs(point2_diff(current_node->coordinate, *crd));
            if (distance <= obj->config.node_remapping_distance) {
                out_nodes[found_nodes] = current_node;
                found_nodes += 1;
                if (found_nodes ==
                    PATHFINDING_GET_ARRAY_OF_CLOSEST_NODES_MAX_NUM) {
                    break;
                }
            }
        }
...
void remap_nodes_to_new_node_if_closer_to_start(pathfinding_object_t* obj,
    obstacle_holder_t* ob_hold, path_node_t** nodes, uint8_t nb_nodes,
    path_node_t* new_node) {
    for (size_t i = 0; i < nb_nodes; i++) {
        point2_t out_crd;
        int err = get_closest_point_of_collision(obj, ob_hold,
            &new_node->coordinate, &nodes[i]->coordinate, &out_crd);
        if (err) {
            continue;
        }
        float total_distance =
            vec2_abs(point2_diff(new_node->coordinate, nodes[i]->coordinate)) +
...
int get_new_valid_coordinates(pathfinding_object_t* obj,
    point2_t* crd_tree_node, point2_t* crd_random_node,
    ...
    crd_new_node->x =
        crd_tree_node->x + (crd_random_node->x - crd_tree_node->x) * frac;
    crd_new_node->y =
        crd_tree_node->y + (crd_random_node->y - crd_tree_node->y) * frac;
    return PATHFINDING_ERROR_NONE;
}
    ...
    for (size_t index_obstacle = 0;
        ...
        if (current_ob->type != obstacle_type_none) {
            ...
            if (status == 1) {
                ...
                float dist = vec2_abs(
                    point2_diff(obstacle_checked_crd, *rand_coordinates));
 ...
 *
 */
uint8_t check_collision(const pathfinding_object_t* obj,
...
point2_t pathfinding_generate_rand_coordinates(
    const pathfinding_object_t* obj, const point2_t* end) {
    uint8_t new_crd_on_goal =
        (utils_get_rand32() & 0x000000FF) < obj->config.rand_goal_probability;
...
point2_t pathfinding_generate_rebuild_rand_coordinates(
    const pathfinding_object_t* obj, const point2_t* end) {
    ...
    if (point_gen_type_probability < obj->config.rand_goal_probability) {
        rand_coordinates = *end;
                   ...
                   obj->config.rand_goal_probability +
                       obj->config.rand_waypoint_probability) &&
...
int pathfinding_cache_waypoint(
    ...
    while (current_node->parent_node != NULL) {
        if (obj->nb_of_coordinate_cached ==
            PATHFINDING_MAX_NUM_OF_COORDINATES_CACHE) {
            return -1;
        }
        obj->coordinate_cache[obj->nb_of_coordinate_cached] =
...
int pathfinding_find_path(pathfinding_object_t* obj, obstacle_holder_t* ob_hold,
    ...
    *end_node = NULL;
    ...
    for (obj->next_free_node_nb = 1;
         obj->next_free_node_nb < PATHFINDING_MAX_NUM_OF_NODES;
        ...
        uint8_t nb_of_to_be_remaped_nodes = get_array_of_closest_node(
            obj, &new_coordinates, to_be_remaped_nodes);
        ...
        current_node->distance_to_start =
            ...
            vec2_abs(point2_diff(
                closest_node_p->coordinate, current_node->coordinate));
...
int pathfinding_rebuild(pathfinding_object_t* obj, obstacle_holder_t* ob_hold,
    const point2_t* start, const point2_t* end, path_node_t** end_node) {
    *end_node = NULL;
    // TODO: Check input validity, must be between 0 and pathfinding_boundaries
    // Init start node
    ...
    for (obj->next_free_node_nb = 1;
        ...
        path_node_t* current_node = &obj->nodes[obj->next_free_node_nb];
        point2_t rand_coordinates =
            pathfinding_generate_rebuild_rand_coordinates(obj, end);
        path_node_t* closest_node_p = get_closest_node(obj, &rand_coordinates);
...
int pathfinding_optimize_path(pathfinding_object_t* obj,
    obstacle_holder_t* ob_hold, path_node_t* solved_path_end_node,
    uint16_t nb_of_nodes_to_add) {
    uint16_t counter = 0;
    path_node_t*
        to_be_remaped_nodes[PATHFINDING_GET_ARRAY_OF_CLOSEST_NODES_MAX_NUM] = {
            0};

    for (; obj->next_free_node_nb < PATHFINDING_MAX_NUM_OF_NODES;
         obj->next_free_node_nb++) {
        path_node_t* current_node = &obj->nodes[obj->next_free_node_nb];
        if (current_node->is_used) {
            uint8_t nb_of_to_be_remaped_nodes = get_array_of_closest_node(
                obj, &current_node->coordinate, to_be_remaped_nodes);
            remap_nodes_to_new_node_if_closer_to_start(obj, ob_hold,
                to_be_remaped_nodes, nb_of_to_be_remaped_nodes, current_node);
            continue;
        }
        ...
        int err = get_closest_point_of_collision(obj, ob_hold,
            &rand_coordinates, &closest_node_p->coordinate, &path_free_crd);
        if (err) {
            continue;
        }
