...
void remap_nodes_to_new_node_if_closer_to_start(pathfinding_object_t* obj, obstacle_holder_t* ob_hold,
    ...
    for (size_t i = 0; i < nb_nodes; i++) {
        ...
        float total_distance =
            vec2_abs(point2_diff(new_node->coordinate, nodes[i]->coordinate)) + new_node->distance_to_start;
    ...
    for (size_t index_obstacle = 0; index_obstacle < OBSTACLE_HOLDER_MAX_NUMBER_OF_OBSTACLE; index_obstacle++) {
        ...
        if (current_ob->type != obstacle_type_none) {
            ...
            if (status == 1) {
                ...
                // closest_node_p->coordinate.x, closest_node_p->coordinate.y, rand_coordinates.x, rand_coordinates.y,
                // path_free_crd.x, path_free_crd.y);
            } else if (status == 2) {
                collision_happened = 1;
                break;
            }
...
point2_t pathfinding_generate_rand_coordinates(const pathfinding_object_t* obj, const point2_t* end) {
...
point2_t pathfinding_generate_rebuild_rand_coordinates(const pathfinding_object_t* obj, const point2_t* end) {
...
int pathfinding_find_path(pathfinding_object_t* obj, obstacle_holder_t* ob_hold, const point2_t* start,
    ...
    *end_node = NULL;
    ...
    if (start->x < obj->config.field_boundaries.min_x || start->y < obj->config.field_boundaries.min_y ||
        start->x >= obj->config.field_boundaries.max_x || start->y >= obj->config.field_boundaries.max_y) {
...
int pathfinding_optimize_path(pathfinding_object_t* obj, obstacle_holder_t* ob_hold, path_node_t* solved_path_end_node,
    ...
    for (; obj->next_free_node_nb < PATHFINDING_MAX_NUM_OF_NODES; obj->next_free_node_nb++) {
        ...
        // the first unused nodes when nb_of_nodes_to_add = 0
        point2_t rand_coordinates;
        rand_coordinates.x =
            obj->config.field_boundaries.min_x +
            utils_get_rand32() % (obj->config.field_boundaries.max_x - obj->config.field_boundaries.min_x);
        rand_coordinates.y =
            obj->config.field_boundaries.min_y +
            utils_get_rand32() % (obj->config.field_boundaries.max_y - obj->config.field_boundaries.min_y);
        path_node_t* closest_node_p = get_closest_node(obj, &rand_coordinates);
...
void pathfinding_debug_write_found_path(pathfinding_object_t* obj, path_node_t* end_node, char* file_path) {
    ...
    uint8_t(*tab)[obj->config.field_boundaries.max_x] =
        malloc(obj->config.field_boundaries.max_y * obj->config.field_boundaries.max_x * sizeof(uint8_t));
    uint8_t path_valid = 0;
    path_node_t* current_node = end_node;
    for (size_t i = 0; i < PATHFINDING_MAX_NUM_OF_NODES; i++) {
